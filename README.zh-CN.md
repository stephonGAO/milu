...
**用国产大模型，3 行代码起一个生产级多用户 Agent 服务。**

一套接口接入 9 家 LLM 提供商 · 内置工具与 MCP · 子代理 · 技能 · RAG 知识库 · 定时任务 · 多用户并发池
...
多数 agent 框架把国产模型当二等公民，且止步于单用户 demo。milu 从它们止步的地方开始：
...
- 🏭 **一个库走完 demo 到生产** —— `AgentPool` 提供 per-user 实例隔离、LRU/TTL 淘汰、全局并发限流、共享 MCP 进程。每个框架都留给你自己解决的问题——*「demo 跑通了，怎么让 100 个用户同时用且会话互不串扰？」*——这里有现成答案，1100+ 测试兜底。
...
> 每个 ✅/❌ 表示截至 2026 年 6 月「库内直接提供」的能力——多数空格可以用外部平台或自研代码补齐。

## 你可以用它做什么

- **基于自有文档的客服机器人** —— RAG 知识库 + 前置自动检索，回答区分内部资料与网络来源，访客间多用户隔离。
- **团队内部多用户助手** —— `docker compose up -d`，每人独立的会话、记忆与知识库。
- **晨报 Agent** —— cron 定时任务，结果送达 Web 界面、outbox 文件或桌面通知。
- **多租户 SaaS Agent** —— `KeyedLLMProvider` 把租户映射到各自的 API Key，资源池保证实例与并发不变量。
