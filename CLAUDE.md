...
- **媒体接入 `media.py`**：各渠道收到**图片**类消息时用平台 API 下载二进制（微信 `media/get` / 飞书 `messages/:id/resources/:key?type=image` / Telegram `getFile`+`/file/bot…`），经 `save_image` 落到「**该用户工作区下 `_incoming` 子目录**」（与 agent_factory 同款 `channel:user_id` 派生工作区，使 strict 部署 `workspace_jail` 下文件工具仍读得到），把绝对路径填进 `InboundMessage.images`；AgentRunner `_compose_input` 经 `agent.run(images=...)` 走视觉物化（图片不经文件工具、不受围栏限制），只发图片无配文时补默认指令 `DEFAULT_IMAGE_PROMPT`（否则 `run("")` 无可回应文本）。落盘带毫秒时间戳前缀防同名覆盖、超 `MAX_IMAGE_BYTES`(10MB) 跳过、按 content-type/文件名推断扩展名（视觉层靠扩展名识别格式）、`media_dir` 每次顺带清理 7 天过期下载。飞书 webhook 与长连接共用 `save_image_resource`。下载失败/超限**整段不抛**、只记日志并跳过该条
