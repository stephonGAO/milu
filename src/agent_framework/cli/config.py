"""CLI 配置：~/.agent_framework/config.json 的读写，及厂商/模型/Key 的解析。

配置文件落在 user_data_dir()（默认 ~/.agent_framework，可被环境变量
AGENT_FRAMEWORK_HOME 覆盖），与会话目录同源、与 CWD 解耦。
"""
...
@dataclass
class CLIConfig:
    """持久化的 CLI 配置（~/.agent_framework/config.json）。"""
