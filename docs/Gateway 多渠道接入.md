...
用户在 IM 里发**图片**时，网关会用各平台 API 把图片下载到本地，交给（支持视觉的）
模型「看」：

- 三渠道分别经微信 `media/get` / 飞书 `messages/:id/resources` / Telegram `getFile`
  下载，落到**该用户工作区下的 `_incoming/` 子目录**，把绝对路径填进消息一并跑过 Agent。
- 图片走视觉物化（base64 注入），**不经文件工具**，故 `strict` 部署的工作区围栏不影响它。
- 只发图片、没配文字时，会自动补一句「请查看用户发来的图片并回应」，模型据图作答。
- 需要**配一个支持视觉的模型**（如 `qwen-vl-plus`）；不支持视觉的模型会降级为纯文本提示。
- 单图上限 10MB，超限或下载失败会被静默跳过（不影响其它消息）。下载的图片保留 7 天后清理。

> 多张图片（如相册）：Telegram 相册按多条更新到达，逐条处理；微信/飞书逐条消息各带一图。
